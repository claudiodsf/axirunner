...
# NO IMPORTS FROM PYAXITRA OR FUTURE IN THIS FILE! (file gets used at
# installation time)
import io
import os
import inspect
...
PYAXITRA_ROOT = os.path.abspath(os.path.join(script_dir, os.pardir))
...
def call_git_describe(abbrev=4):
    try:
        p = Popen(['git', 'rev-parse', '--show-toplevel'],
                  cwd=PYAXITRA_ROOT, stdout=PIPE, stderr=PIPE)
        p.stderr.close()
        path = p.stdout.readline().decode().strip()
        p.stdout.close()
    except Exception:
        return None
    if os.path.normpath(path) != PYAXITRA_ROOT:
        return None
    try:
